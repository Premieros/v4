...
export function WasteCenterPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (branchFilter) {
        entryQ = entryQ.eq('branch_id', branchFilter);
      }
      ...
      const unitQ = supabase.from('inventory_units').select('id, name, cost_price, unit_type').eq('is_active', true).order('name');
      ...
      if (branchFilter) {
        ...
        prodQ = prodQ.or(`branch_id.eq.${branchFilter},branch_id.is.null`);
        whQ = whQ.eq('branch_id', branchFilter);
      }

      const [catRes, entryRes, rmRes, prodRes, unitRes, whRes] = await Promise.all([
        catQ,
        entryQ,
        rmQ,
        prodQ,
  ...
  const handleRawMaterialChange = (rmId: string) => {
    ...
    setForm((f) => ({
      ...
      unit_cost: selected?.cost_price ? Number(selected.cost_price) : f.unit_cost,
    }));
  };
  ...
  const handleCreate = async () => {
    ...
    if (!branchFilter) {
      show(ar ? 'اختر الفرع أولاً من أعلى الصفحة' : 'Select a branch first from the top bar', 'error');
      return;
    }
    ...
    try {
      ...
      show(ar ? 'تم تسجيل الهالك بنجاح' : 'Waste entry recorded successfully', 'success');
      setShowForm(false);
      setForm(EMPTY_FORM);
      void load();
    } catch (err) {
  ...
  const statusColor = (s: string) =>
    s === 'approved' ? 'text-ui-success' : s === 'rejected' ? 'text-ui-danger' : 'text-ui-warning';

  const getItemName = (r: WasteEntry) => {
    const entryAny = r as unknown as {
      raw_material?: { name?: string };
      product?: { name?: string };
      inventory_unit?: { name?: string };
    };
    if (entryAny.raw_material?.name) {
      return (
        <span className="inline-flex items-center gap-1">
          <span className="px-1.5 py-0.5 rounded text-xs bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">
            {ar ? 'خامة' : 'Raw Material'}
          </span>
          <span className="font-medium text-ui-text">{entryAny.raw_material.name}</span>
        </span>
      );
    }
    if (entryAny.product?.name) {
      return (
        <span className="inline-flex items-center gap-1">
          <span className="px-1.5 py-0.5 rounded text-xs bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
            {ar ? 'منتج' : 'Product'}
          </span>
          <span className="font-medium text-ui-text">{entryAny.product.name}</span>
        </span>
      );
    }
    if (entryAny.inventory_unit?.name) {
      return (
        <span className="inline-flex items-center gap-1">
          <span className="px-1.5 py-0.5 rounded text-xs bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400">
            {ar ? 'وحدة' : 'Unit'}
          </span>
          <span className="font-medium text-ui-text">{entryAny.inventory_unit.name}</span>
        </span>
      );
    }
  ...
  const baseColumns: Column<WasteEntry>[] = [
    ...
    {
      key: 'warehouse',
      header: ar ? 'المخزن' : 'Warehouse',
      render: (r) => {
        const wh = (r as unknown as { warehouse?: { name?: string } }).warehouse;
        return wh?.name ?? '-';
      },
    },
            ...
            r.status === 'pending' ? (
              <div className="flex gap-1">
                <button
                  ...
                  className="text-ui-success hover:text-ui-success p-1 rounded hover:bg-emerald-50 dark:hover:bg-emerald-950/30"
                ...
                <button
                  ...
                  className="text-ui-danger hover:text-ui-danger p-1 rounded hover:bg-rose-50 dark:hover:bg-rose-950/30"
                  title={ar ? 'رفض' : 'Reject'}
                >
                  <X className="h-4 w-4" />
                </button>
  ...
  return (
    <DesignSurface testId="waste-center">
      <DesignPageHeader
        ...
        subtitle={ar ? 'تسجيل ومراجعة هالك المواد الخام والمنتجات والإنتاج بالمخازن' : 'Record and review waste for raw materials, products, and inventory.'}
      />
      <div className="space-y-4">
        {!branchFilter && (
          <div className="flex items-center gap-2 p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-200 text-sm">
            ...
            <span>{ar ? 'تنبيه: يُرجى اختيار فرع من شريط الفروع أعلى الشاشة لتسجيل أو عرض هالك الفرع.' : 'Notice: Please select a branch from the top header to view or record waste.'}</span>
          </div>
        )}
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={ar ? 'تسجيل هالك جديد' : 'Record Waste'} size="lg">
        <div className="space-y-4">
            ...
            <Select
              ...
              onChange={(e) => {
                ...
                setForm((f) => ({
                  ...f,
                  waste_type: newType,
                  raw_material_id: newType === 'raw_material' ? f.raw_material_id : '',
                  product_id: newType === 'finished_good' || newType === 'damaged' || newType === 'expired' ? f.product_id : '',
                  inventory_unit_id: newType === 'production' ? f.inventory_unit_id : '',
                }));
              }}
            >
              {WASTE_TYPES.map((wt) => (
          ...
          {form.waste_type === 'raw_material' && (
            <div className="p-3 rounded-xl border border-amber-200 dark:border-amber-800/40 bg-amber-50/40 dark:bg-amber-950/10 space-y-2">
              <label className="text-xs font-semibold text-amber-900 dark:text-amber-300">
                {ar ? 'تحديد المادة الخام التالفة' : 'Select Raw Material'}
              </label>
              <Select
                value={form.raw_material_id}
                onChange={(e) => handleRawMaterialChange(e.target.value)}
              >
                <option value="">{ar ? '-- اختر المادة الخام --' : '-- Select Raw Material --'}</option>
                {rawMaterials.map((m) => (
                  <option key={m.id} value={m.id}>
                    {m.name} {m.cost_price ? `(${m.cost_price} EGP)` : ''}
                  </option>
                ))}
              </Select>
            </div>
          ...
          {(form.waste_type === 'finished_good' || form.waste_type === 'damaged' || form.waste_type === 'expired') && (
            <div className="p-3 rounded-xl border border-blue-200 dark:border-blue-800/40 bg-blue-50/40 dark:bg-blue-950/10 space-y-2">
              <label className="text-xs font-semibold text-blue-900 dark:text-blue-300">
                {ar ? 'تحديد المنتج التالف' : 'Select Product'}
              </label>
              <Select
              ...
              >
                ...
                {products.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.name} {p.cost_price ? `(${p.cost_price} EGP)` : ''}
                  </option>
                ))}
              </Select>
            </div>
          ...
          {form.waste_type === 'production' && (
            <div className="p-3 rounded-xl border border-purple-200 dark:border-purple-800/40 bg-purple-50/40 dark:bg-purple-950/10 space-y-2">
              <label className="text-xs font-semibold text-purple-900 dark:text-purple-300">
                {ar ? 'تحديد وحدة الإنتاج / التصنيع' : 'Select Inventory Unit / Recipe Item'}
              </label>
              <Select
                value={form.inventory_unit_id}
                onChange={(e) => handleInventoryUnitChange(e.target.value)}
              >
                <option value="">{ar ? '-- اختر وحدة الإنتاج --' : '-- Select Production Unit --'}</option>
                {inventoryUnits.map((u) => (
                  <option key={u.id} value={u.id}>
                    {u.name} {u.cost_price ? `(${u.cost_price} EGP)` : ''}
                  </option>
                ))}
              </Select>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <Input
              label={ar ? 'تكلفة الوحدة' : 'Unit Cost'}
              type="number"
              step="0.01"
              min="0"
          ...
          <div className="p-2 rounded-lg bg-ui-page-alt text-xs flex justify-between items-center">
            <span className="text-ui-muted">{ar ? 'إجمالي تكلفة الهالك المتوقعة:' : 'Calculated Total Cost:'}</span>
            <span className="font-bold text-ui-text text-sm">
              {(form.quantity * form.unit_cost).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} EGP
            </span>
          </div>

          <Textarea
            label={ar ? 'السبب / الملاحظات' : 'Reason / Notes'}
            value={form.reason}
            onChange={(e) => setForm((f) => ({ ...f, reason: e.target.value }))}
            rows={2}
            placeholder={ar ? 'مثال: تلف أثناء التحضير، انتهاء صلاحية، كسر...' : 'e.g. damaged during preparation, expired, broken...'}
          />
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-ui-border">
            ...
            <Button onClick={() => void handleCreate()}>
              {ar ? 'حفظ وتسجيل الهالك' : 'Save & Record Waste'}
            </Button>
          </div>
        </div>
      </Modal>
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        /* report is optional, fail silently */
      }
      setLoading(false);
    })();
  }, [branchFilter]);
